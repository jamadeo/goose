...
impl ApiClient {
    ...
    pub fn with_timeout(host: String, auth: AuthMethod, timeout: Duration) -> Result<Self> {
        Self::with_timeout_and_tls_config(host, auth, timeout, None)
    }

    pub fn with_timeout_and_tls_config(
        host: String,
        auth: AuthMethod,
        timeout: Duration,
        tls_config: Option<TlsConfig>,
    ...
    async fn get_oauth_token(&self, _config: &OAuthConfig) -> Result<String> {
        anyhow::bail!("OAuth auth method requires a provider-specific auth provider")
    }
}
