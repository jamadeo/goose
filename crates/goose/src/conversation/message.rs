...
use chrono::Utc;
pub use goose_types::{
    ActionRequired, ActionRequiredData, FrontendToolRequest, InferenceMetadata, MessageContent,
    ...
    TokenState, ToolCallResult, ToolConfirmationRequest, ToolRequest, ToolResponse, ToolResult,
    TOOL_META_CHAIN_SUMMARY_KEY, TOOL_META_EXTERNAL_DISPATCH_KEY, TOOL_META_TITLE_KEY,
};
use rmcp::model::{
    ...
    fn from(prompt_message: PromptMessage) -> Self {
        ...
        let content = match prompt_message.content {
            ...
            PromptMessageContent::Resource { resource } => {
                MessageContent::text(goose_types::extract_text_from_resource(&resource.resource))
            }
        };
