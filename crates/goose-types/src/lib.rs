...
mod tests {
    ...
    #[test]
    fn conversation_push_merges_same_message_text_and_thinking() {
        let mut conversation = Conversation::empty();
        conversation.push(Message::assistant().with_text("Hello").with_id("turn-1"));
        conversation.push(Message::assistant().with_text(" world").with_id("turn-1"));
        ...
        conversation.push(
            Message::assistant()
                .with_thinking("should think.", "sig")
                .with_id("turn-2"),
        );

        assert_eq!(conversation.messages().len(), 1);
        let thinking = conversation.messages()[0].content[0].as_thinking().unwrap();
        assert_eq!(thinking.thinking, "I should think.");
        assert_eq!(thinking.signature, "sig");
    }

    #[test]
    fn fix_conversation_removes_invalid_tool_flow() {
        let messages = vec![
            Message::assistant()
                .with_text("I'll search")
        ...
        assert_eq!(fixed.messages()[0].role, Role::User);
        assert_eq!(fixed.messages()[0].as_concat_text(), "Hello");
        assert!(issues
            .iter()
            .any(|issue| issue == "Removed orphaned tool request 'search_1'"));
        assert!(issues
            .iter()
            .any(|issue| issue == "Removed orphaned tool response 'wrong_id'"));
    }

    #[test]
    fn conversation_filters_visible_messages() {
        let conversation = Conversation::new_unvalidated([
            Message::user().with_text("visible"),
            Message::user().with_text("agent only").agent_only(),
        ...
        assert_eq!(conversation.agent_visible_messages().len(), 2);
        assert_eq!(conversation.user_visible_messages().len(), 2);
    }
